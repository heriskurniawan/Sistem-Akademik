...
function cmb_dinamis($name, $table, $field, $pk, $selected = "PILIH", $extra = null) {
    $ci = & get_instance();
...
function cmb_dinamis2($name, $id, $table, $field, $pk, $selected = null, $extra = null) {
    $ci = & get_instance();
    $cmb = "<select name='$name' id='$id' class='form-control select2' $extra>";
    $data = $ci->db->get($table)->result();
    $cmb .= "<option selected='true' disabled='disabled'>---Select---</option>";
    foreach ($data as $row) {
        $cmb .="<option value='" . $row->$pk . "'";
        $cmb .= $selected == $row->$pk ? 'selected' : '';
        $cmb .=">" . $row->$field . "</option>";
